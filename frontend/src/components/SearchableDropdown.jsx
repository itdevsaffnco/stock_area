...
const SearchableDropdown = ({ options, value, onChange, placeholder, labelKey = 'label', valueKey = 'value', disabled = false, required = false }) => {
    ...
    if (disabled) {
        return (
            <div className="w-full border p-2 rounded bg-gray-100 text-gray-400 h-[42px] flex items-center">
                {placeholder}
            </div>
        );
    }
    ...
    return (
        <div className="relative w-full" ref={wrapperRef}>
            <div 
                className={`border p-2 rounded bg-white cursor-pointer flex justify-between items-center h-[42px] ${required && !value ? 'border-red-300' : 'border-gray-300'}`}
