...
import { storeService, productService, stockService, authService } from "../services";
...
function StaffDashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [storesRes, productsRes] = await Promise.all([storeService.getAll(), productService.getAll()]);
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <select
                        ...
                        value={destSubChannel}
                        onChange={(e) => {
                          setDestSubChannel(e.target.value);
                          setDestStoreId("");
                        }}
                        disabled={!destChannel}
                        required
                      >
                        <option value="">Select Sub Channel</option>
                        {availableDestSubChannels.map((s) => (
                          <option key={s} value={s}>
                            {s}
                          </option>
                        ))}
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700">SKU Code</label>
                  <input type="text" className="mt-1 block w-full rounded-md border-gray-300 bg-gray-100 text-gray-500 p-2 border cursor-not-allowed" value={skuCode} disabled placeholder="Auto-filled Code" />
                </div>
              </div>
